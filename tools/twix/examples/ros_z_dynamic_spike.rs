...
use ros_z::{
    ...
    dynamic::{DynamicMessage, DynamicValue, EnumPayloadValue, EnumValue},
};
...
async fn main() -> Result<()> {
    ...
    let subscriber = node
        .create_dyn_sub_auto(&args.topic, discovery_timeout)
        .await
        .map_err(|error| eyre!(error))
    ...
    println!("source gid: {:?}", received.source_gid);
    println!(
        "json: {}",
        serde_json::to_string_pretty(&dynamic_message_to_json(&received.message))?
    );
...
async fn wait_for_topic(
    context: &ros_z::context::ZContext,
    topic: &str,
    timeout: Duration,
) -> Result<()> {
