...
impl ros_z::msg::ZMessage for MathSnapshot {
    type Serdes = ros_z::msg::SerdeCdrSerdes<Self>;
}

impl ros_z::msg::ZMessage for MathCommand {
    type Serdes = ros_z::msg::SerdeCdrSerdes<Self>;
}

impl ros_z::msg::ZMessage for MotionMode {
    type Serdes = ros_z::msg::SerdeCdrSerdes<Self>;
}
...
fn standard_nalgebra_schema_is_basic_only() {
    ...
    assert_eq!(isometry_schema.type_name, "nalgebra/msg/Isometry3F32");
}
...
async fn nalgebra_fields_roundtrip_via_standard_discovery() {
    ...
    let publisher = pub_node
        .create_pub::<MathSnapshot>("/math_snapshot")
        .build()
        .expect("publisher");
    ...
    let publish_task = tokio::spawn(async move {
        for _ in 0..20 {
            publisher.publish(&expected).expect("publish");
    ...
    let subscriber = sub_node
        .create_dyn_sub_auto("/math_snapshot", Duration::from_secs(10))
        .await
...
async fn extended_envelope_uses_extended_discovery_with_basic_nalgebra_fields() {
    ...
    let publisher = pub_node
        .create_pub::<MathCommand>("/math_command")
        .build()
        .expect("publisher");
    ...
    let sub_node = sub_ctx
        ...
        .expect("subscriber node");
    let subscriber = sub_node
        .create_dyn_sub_auto("/math_command", Duration::from_secs(10))
        .await
        .expect("subscriber discovery")
        .build()
        .expect("subscriber build");
