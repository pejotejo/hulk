...
impl TopicAccumulator {
    fn from_channel(channel: &Channel, schema: Option<&SchemaInfo>) -> Self {
        let mut topic = Self {
            ...
            type_hash: channel.metadata.get("type_hash").cloned(),
    ...
    fn update_from_channel(&mut self, channel: &mcap::Channel<'_>) {
        ...
        self.type_hash = self
            .type_hash
            .clone()
            .or_else(|| channel.metadata.get("type_hash").cloned());
    ...
    fn merge_resolved_topic(&mut self, topic: &ResolvedTopic) {
        ...
        self.type_hash = self
            .type_hash
            .clone()
            .or_else(|| Some(topic.type_hash.clone()));
    }
...
fn collect_static_records(bytes: &[u8]) -> Result<StaticRecords> {
    ...
    let header = header.ok_or_else(|| anyhow::anyhow!("MCAP file did not contain a header"))?;
