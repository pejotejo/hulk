...
pub async fn run(
    ...
    let options = build_record_options(router, domain, backend, args)?;
...
fn build_record_options(
    router: &str,
    domain: usize,
    backend: Backend,
    args: &RecordArgs,
) -> Result<RecorderOptions> {
    ...
    Ok(RecorderOptions {
        ...
        duration_limit,
        stats_interval,
        session_metadata: session_metadata(router, domain, backend),
    })
}
...
fn session_metadata(router: &str, domain: usize, backend: Backend) -> BTreeMap<String, String> {
    BTreeMap::from([
        ("router".to_string(), router.to_string()),
        ("domain".to_string(), domain.to_string()),
        ("backend".to_string(), backend_name(backend).to_string()),
    ])
}

fn backend_name(backend: Backend) -> &'static str {
    match backend {
        Backend::RmwZenoh => "rmw-zenoh",
        Backend::Ros2Dds => "ros2dds",
    }
}
